...
plt = plot_bands(bcc_kpath, hartree2ev * (bands_bcc .- ϵf_bcc), color=:red, fermi_level=0.)
ylims!(-15, 30)
...
plt = plot_bands(fcc_kpath, hartree2ev * (bands_fcc .- ϵf_fcc), color=:red, fermi_level=0.)
ylims!(-20, 30)
...
# ord-2 offsite
function band_plot(i, phase::Symbol; ylabel="Energy (eV)", ylims=(-20, 25), Hp_NMM=nothing, Sp_NMM=nothing)
    onsite_label, offsite_label = labels(phase)
    base = phase == :FCC ? "hybridon" : "hybridonoff"
    
    if Hp_NMM === nothing && Sp_NMM === nothing
        Hp_NMM, Sp_NMM = h5open("out_hyboff_2_6_2_$(i)_1_$(i+2)$onsite_label.h5") do h5file
            real.(permutedims(read(h5file, "H"), (3, 1, 2))), real.(permutedims(read(h5file, "S"), (3, 1, 2)))
        end
        Hp2_NMM, Sp2_NMM = h5open("out_$(base)_2_6_2_6_1_8$offsite_label.h5") do h5file
            real.(permutedims(read(h5file, "H"), (3, 1, 2))), real.(permutedims(read(h5file, "S"), (3, 1, 2)))
        end
    ...
    plot_bands!(plt, kpath, hartree2ev * bands_p, ϵ = hartree2ev * err_p, fermi_level=hartree2ev * ϵf_p, color=:blue, label="hybrid_onsite(2,6) + hybrid_offsite(2,$i)")
...
function band_plot_new(onsite_order, onsite_degree, offsite_order, offsite_degree, phase::Symbol; ylabel="Energy (eV)", ylims=(-20, 25), Hp_NMM=nothing, Sp_NMM=nothing)
    if Hp_NMM === nothing && Sp_NMM === nothing
        ...
        Hp_NMM, Sp_NMM = h5open("out_hyboff_2_6_$(offsite_order)_$(offsite_degree)_1_$(offsite_degree+2)$phase_label.h5") do h5file
            real.(permutedims(read(h5file, "H"), (3, 1, 2))), real.(permutedims(read(h5file, "S"), (3, 1, 2)))
        end
        Hp2_NMM, Sp2_NMM = h5open("out_hyb_$(onsite_order)_$(onsite_degree)_1_6$phase_label.h5") do h5file
            real.(permutedims(read(h5file, "H"), (3, 1, 2))), real.(permutedims(read(h5file, "S"), (3, 1, 2)))
        end
        Hp_NMM[365,:,:] = Hp2_NMM[365,:,:]
        Sp_NMM[365,:,:] = I(14) # on site overlap
    ...
    bands_p, err_p = bandstructure(kpath, Hp_NMM, Sp_NMM, R; H0_NMM=H_NMM, S0_NMM=S_NMM)
    plt = plot_bands(kpath, hartree2ev * bands, color=:red, fermi_level=hartree2ev * ϵf, label="DFT", ylabel=ylabel)
    plot_bands!(plt, kpath, hartree2ev * bands_p,  fermi_level=hartree2ev * ϵf_p, ϵ = hartree2ev * err_p, color=:blue, label="hybrid_onsite($onsite_order,$onsite_degree) + hybrid_offsite($offsite_order,$offsite_degree)")
    return (ylims!(ylims...), bands_p, ϵf_p)
end
...
anim = @animate for i ∈ 6:12
    ...
    push!(band_error_bcc_order2, band_error(bands, bands_bcc, ϵf, ϵf_bcc))
    plt
end
...
anim_fcc = @animate for i ∈ 6:12
    ...
    push!(band_error_fcc_order2, band_error(bands, bands_fcc, ϵf, ϵf_fcc))
    plt
end
...
anim = @animate for i ∈ 6:14
    ...
    push!(band_error_bcc_order1, band_error(bands, bands_bcc, ϵf, ϵf_bcc))
    plt
end
...
anim_fcc = @animate for i ∈ 6:14
    ...
    push!(band_error_fcc_order1, band_error(bands, bands_fcc, ϵf, ϵf_fcc))
    plt
end
...
function dos_plot(i, phase::Symbol; Hp_NMM=nothing, Sp_NMM=nothing, xlabel=L"Energy $E$ / eV", ylabel="DoS", swap_xy=false)
    onsite_label, offsite_label = labels(phase)
    base = phase == :FCC ? "hybridon" : "hybridonoff"
    ...
    if swap_xy
        hline!([ϵf * hartree2ev], linestyle=:dash, label=nothing, color=:red)
    else
        vline!([ϵf * hartree2ev], linestyle=:dash, label=nothing, color=:red)
    end

    x, y = ϵgrid * hartree2ev, d_p / hartree2ev
    if swap_xy
        x, y = y, x
    end
    plot!(x, y, lw=1.5, label="ACEtb max degree $i", 
          color=:blue)
    if swap_xy
        hline!([ϵf_p * hartree2ev], linestyle=:dash, label="degree $i", color=:blue)
        ylims!(-20, 10)
        xlims!(0, 0.6)    
    else
        vline!([ϵf_p * hartree2ev], linestyle=:dash, label="degree $i", color=:blue)
        xlims!(-20, 10)
        ylims!(0, 0.6)    
    end
    #title!("Degree $i")
...
errors = Dict()
for errorfile in glob("off_model_hybrid_1115/*_errors.json")
    modelID = split(splitpath(errorfile)[end], "_")[1]
    errors[modelID] = JSON.parsefile(errorfile)
end

errors["tuned"] = JSON.parsefile("tuned-model/2606434113214136884_errors.json")
...
errors = Dict()
for errorfile in glob("on_model_hybrid_1120_correctmean/*_errors.json")
    modelID = split(splitpath(errorfile)[end], "_")[1]
    errors[modelID] = JSON.parsefile(errorfile)
end
...
# FIXME this should be averaged across the training/test database
...
# # Offsite H(1,12) + S(1,14)
# ord = 1
# i = 14
# Hp_NMM_bcc, Sp_NMM_bcc = h5open("out_hyboff_2_6_$(ord)_$(i)_1_$(i+2)_bcc.h5") do h5file
#     real.(permutedims(read(h5file, "H"), (3, 1, 2))), real.(permutedims(read(h5file, "S"), (3, 1, 2)))
# end

# # Onsite H(2,4) + S(2,4)
# i = 4
# Hp2_NMM_bcc, Sp2_NMM_bcc = h5open("out_hyb_2_$(i)_1_6_bcc.h5") do h5file
#     real.(permutedims(read(h5file, "H"), (3, 1, 2))), real.(permutedims(read(h5file, "S"), (3, 1, 2)))
# end
# Hp_NMM_bcc[365,:,:] = copy(Hp2_NMM_bcc[365,:,:])
# Sp_NMM_bcc[365,:,:] = copy(Sp2_NMM_bcc[365,:,:])


# # off-site modification
# # pp modification (1,13) for all 4 blocks

# Hp2_NMM_bcc, Sp2_NMM_bcc = h5open("out_hyboff_2_6_1_13_1_15_bcc.h5") do h5file
#     real.(permutedims(read(h5file, "H"), (3, 1, 2))), real.(permutedims(read(h5file, "S"), (3, 1, 2)))
# end
# Hp_NMM_bcc[1:364,4:9,4:9] = Hp2_NMM_bcc[1:364,4:9,4:9]
# Hp_NMM_bcc[366:729,4:9,4:9] = Hp2_NMM_bcc[366:729,4:9,4:9]

# # sd/ds modification
# Hp2_NMM_bcc, Sp2_NMM_bcc = h5open("out_hyboff_2_6_1_11_1_13_bcc.h5") do h5file
#     real.(permutedims(read(h5file, "H"), (3, 1, 2))), real.(permutedims(read(h5file, "S"), (3, 1, 2)))
# end
# Hp_NMM_bcc[1:364,3,10:14] = Hp2_NMM_bcc[1:364,3,10:14]
# Hp_NMM_bcc[366:729,3,10:14] = Hp2_NMM_bcc[366:729,3,10:14]
# Hp_NMM_bcc[1:364,10:14,3] = Hp2_NMM_bcc[1:364,10:14,3]
# Hp_NMM_bcc[366:729,10:14,3] = Hp2_NMM_bcc[366:729,10:14,3]

# # sp+ps modification
# Hp2_NMM_bcc, Sp2_NMM_bcc = h5open("out_hyboff_2_6_1_10_1_12_bcc.h5") do h5file
#     real.(permutedims(read(h5file, "H"), (3, 1, 2))), real.(permutedims(read(h5file, "S"), (3, 1, 2)))
# end
# Hp_NMM_bcc[1:364,3,7:9] = Hp2_NMM_bcc[1:364,3,7:9]
# Hp_NMM_bcc[366:729,3,7:9] = Hp2_NMM_bcc[366:729,3,7:9]
# Hp_NMM_bcc[1:364,7:9,3] = Hp2_NMM_bcc[1:364,7:9,3]
# Hp_NMM_bcc[366:729,7:9,3] = Hp2_NMM_bcc[366:729,7:9,3]
# Hp2_NMM_bcc, Sp2_NMM_bcc = h5open("out_hyboff_2_6_1_12_1_14_bcc.h5") do h5file
#     real.(permutedims(read(h5file, "H"), (3, 1, 2))), real.(permutedims(read(h5file, "S"), (3, 1, 2)))
# end
# Hp_NMM_bcc[1:364,3,4:6] = Hp2_NMM_bcc[1:364,3,4:6]
# Hp_NMM_bcc[366:729,3,4:6] = Hp2_NMM_bcc[366:729,3,4:6]
# Hp_NMM_bcc[1:364,4:6,3] = Hp2_NMM_bcc[1:364,4:6,3]
# Hp_NMM_bcc[366:729,4:6,3] = Hp2_NMM_bcc[366:729,4:6,3]

# # ss modification
# Hp2_NMM_bcc, Sp2_NMM_bcc = h5open("out_hyboff_2_6_1_9_1_11_bcc.h5") do h5file
#     real.(permutedims(read(h5file, "H"), (3, 1, 2))), real.(permutedims(read(h5file, "S"), (3, 1, 2)))
# end
# Hp_NMM_bcc[1:364,3,3] = Hp2_NMM_bcc[1:364,3,3]
# Hp_NMM_bcc[366:729,3,3] = Hp2_NMM_bcc[366:729,3,3]

Hp_NMM_bcc, Sp_NMM_bcc = h5open("out_hyb_tunedoff_2_9_bcc.h5") do h5file
     real.(permutedims(read(h5file, "H"), (3, 1, 2))), real.(permutedims(read(h5file, "S"), (3, 1, 2)))
end

bcc_plt, bands, ϵf = band_plot(0, :BCC, Hp_NMM=Hp_NMM_bcc, Sp_NMM=Sp_NMM_bcc, ylims=(-20, 25))
band_error_bcc_tuned = band_error(bands, bands_bcc, ϵf, ϵf_bcc)
dos_error_bcc_tuned = dos_error(0, :BCC, Hp_NMM=Hp_NMM_bcc, Sp_NMM=Sp_NMM_bcc)
bcc_dos_plt = dos_plot(0, :BCC, Hp_NMM=Hp_NMM_bcc, Sp_NMM=Sp_NMM_bcc, swap_xy=true)
ylims!(bcc_dos_plt, -20, 25)

##

# ## FCC

# # Offsite H(1,14) + S(1,16)
# ord = 1
# i = 14
# Hp_NMM_fcc, Sp_NMM_fcc = h5open("out_hyboff_2_6_$(ord)_$(i)_1_$(i+2).h5") do h5file
#     real.(permutedims(read(h5file, "H"), (3, 1, 2))), real.(permutedims(read(h5file, "S"), (3, 1, 2)))
# end

# # Onsite H(2,4) + S(2,4)
# i = 4
# Hp2_NMM_fcc, Sp2_NMM_fcc = h5open("out_hyb_2_$(i)_1_6.h5") do h5file
#     real.(permutedims(read(h5file, "H"), (3, 1, 2))), real.(permutedims(read(h5file, "S"), (3, 1, 2)))
# end
# Hp_NMM_fcc[365,:,:] = copy(Hp2_NMM_fcc[365,:,:])
# Sp_NMM_fcc[365,:,:] = copy(Sp2_NMM_fcc[365,:,:])

# # off-site modification
# # pp modification - (1,13) for all 4 blocks
# Hp2_NMM_fcc, Sp2_NMM_fcc = h5open("out_hyboff_2_6_1_13_1_15.h5") do h5file
#     real.(permutedims(read(h5file, "H"), (3, 1, 2))), real.(permutedims(read(h5file, "S"), (3, 1, 2)))
# end
# Hp_NMM_fcc[1:364,4:9,4:9] = Hp2_NMM_fcc[1:364,4:9,4:9]
# Hp_NMM_fcc[366:729,4:9,4:9] = Hp2_NMM_fcc[366:729,4:9,4:9]

# # sd/ds modification - (1,11) for the 3rd block
# Hp2_NMM_fcc, Sp2_NMM_fcc = h5open("out_hyboff_2_6_1_11_1_13.h5") do h5file
#     real.(permutedims(read(h5file, "H"), (3, 1, 2))), real.(permutedims(read(h5file, "S"), (3, 1, 2)))
# end
# Hp_NMM_fcc[1:364,3,10:14] = Hp2_NMM_fcc[1:364,3,10:14]
# Hp_NMM_fcc[366:729,3,10:14] = Hp2_NMM_fcc[366:729,3,10:14]
# Hp_NMM_fcc[1:364,10:14,3] = Hp2_NMM_fcc[1:364,10:14,3]
# Hp_NMM_fcc[366:729,10:14,3] = Hp2_NMM_fcc[366:729,10:14,3]

# # sp+ps modification - (1-10) for [3,2]/[2,3] and (1,12) for [3,1]/[1,3]
# Hp2_NMM_fcc, Sp2_NMM_fcc = h5open("out_hyboff_2_6_1_10_1_12.h5") do h5file
#     real.(permutedims(read(h5file, "H"), (3, 1, 2))), real.(permutedims(read(h5file, "S"), (3, 1, 2)))
# end
# Hp_NMM_fcc[1:364,3,7:9] = Hp2_NMM_fcc[1:364,3,7:9]
# Hp_NMM_fcc[366:729,3,7:9] = Hp2_NMM_fcc[366:729,3,7:9]
# Hp_NMM_fcc[1:364,7:9,3] = Hp2_NMM_fcc[1:364,7:9,3]
# Hp_NMM_fcc[366:729,7:9,3] = Hp2_NMM_fcc[366:729,7:9,3]
# Hp2_NMM_fcc, Sp2_NMM_fcc = h5open("out_hyboff_2_6_1_12_1_14.h5") do h5file
#     real.(permutedims(read(h5file, "H"), (3, 1, 2))), real.(permutedims(read(h5file, "S"), (3, 1, 2)))
# end
# Hp_NMM_fcc[1:364,3,4:6] = Hp2_NMM_fcc[1:364,3,4:6]
# Hp_NMM_fcc[366:729,3,4:6] = Hp2_NMM_fcc[366:729,3,4:6]
# Hp_NMM_fcc[1:364,4:6,3] = Hp2_NMM_fcc[1:364,4:6,3]
# Hp_NMM_fcc[366:729,4:6,3] = Hp2_NMM_fcc[366:729,4:6,3]

# # ss modification - (1,9) for [3,3] block
# Hp2_NMM_fcc, Sp2_NMM_fcc = h5open("out_hyboff_2_6_1_9_1_11.h5") do h5file
#     real.(permutedims(read(h5file, "H"), (3, 1, 2))), real.(permutedims(read(h5file, "S"), (3, 1, 2)))
# end
# Hp_NMM_fcc[1:364,3,3] = Hp2_NMM_fcc[1:364,3,3]
# Hp_NMM_fcc[366:729,3,3] = Hp2_NMM_fcc[366:729,3,3]

# Hp_NMM_fcc, Sp_NMM_fcc = h5open("out_hyb_tunedoff_2_9.h5") do h5file
#     real.(permutedims(read(h5file, "H"), (3, 1, 2))), real.(permutedims(read(h5file, "S"), (3, 1, 2)))
# end
...
fcc_plt, bands, ϵf = band_plot(0, :FCC, Hp_NMM=Hp_NMM_fcc, Sp_NMM=Sp_NMM_fcc, ylims=(-20, 25))
band_error_fcc_tuned = band_error(bands, bands_fcc, ϵf, ϵf_fcc)
dos_error_fcc_tuned = dos_error(0, :FCC, Hp_NMM=Hp_NMM_fcc, Sp_NMM=Sp_NMM_fcc)
fcc_dos_plt = dos_plot(0, :FCC, Hp_NMM=Hp_NMM_fcc, Sp_NMM=Sp_NMM_fcc, swap_xy=true)
...
title!(fcc_plt, "")
title!(bcc_plt, "")
ylabel!(fcc_plt, "Energy / eV")
ylabel!(bcc_plt, "Energy / eV")
title!(fcc_dos_plt, "")
title!(bcc_dos_plt, "")
xlabel!(fcc_dos_plt, "")
xlabel!(bcc_dos_plt, "")
...
annotate!(fcc_plt, 0.25, -18.5, text("FCC", font=font(8)))
annotate!(bcc_plt, 0.25, -18.5, text("BCC", font=font(8)))
...
p1 = plot(6:14, dos_error_fcc_order1, label = "FCC order 1", lw=2, color=3, 
     ylabel=L"$\| DoS_\mathrm{DFT} - DoS_\mathrm{ACEtb} \|_W$", grid=false)
plot!(6:12, dos_error_fcc_order2, label = "FCC order 2", lw=2, color=3, ls=:dash)
hline!([dos_error_fcc_tuned], color=3, label="FCC optimal", ls=:dot, lw=2)

plot!(6:14, dos_error_bcc_order1, label = "BCC order 1", lw=2, color=4)
plot!(6:12, dos_error_bcc_order2, label = "BCC order 2", lw=2, color=4, ls=:dash)
hline!([dos_error_bcc_tuned], color=4, label="BCC optimal", ls=:dot, lw=2)
xlims!(6,12)
ylims!(0, 1)



p2 = plot(6:14, band_error_fcc_order1 * hartree2ev, xlabel="Maximum degree", ylabel="RMSE Band Error / eV", 
...
# plt = plot(    heatmap!(p4, log10.(abs.(H_NMM_fcc[365,:,:])),  
#         colorbar=false, clims=(-10, 1), title=L"\mathrm{FCC}\; \mathrm{Onsite}\; \log|H|", titlefont = font(12)),
#         #  heatmap(log10.(abs.(S_NMM_fcc[365,:,:])),  
#         #           clims=(-10, 1), title=L"\mathrm{FCC}\; \mathrm{Onsite}\; \log|H|", titlefont = font(12)), 
#     heatmap!(p5, log10.(abs.(H_NMM_fcc[366,:,:])),  
#         colorbar=false, clims=(-10, 1), title=L"\mathrm{FCC}\; \mathrm{Onsite}\; \log|H|", titlefont = font(12)),
#     heatmap!(p6, log10.(abs.(S_NMM_fcc[366,:,:])), 
#         colorbar=true, clims=(-10, 1), title=L"\mathrm{FCC}\; \mathrm{Offsite}\; \log|S|", titlefont = font(12)),

#     heatmap!(p7, log10.(abs.(H_NMM_fcc[365,:,:] - Hp_NMM_fcc[365,:,:])), 
#               colorbar=false, clims=(-10, 1), title=L"\mathrm{FCC}\; \mathrm{Onsite}\; \log|H - \tilde{H}|", titlefont = font(12)),
#       #  heatmap(log10.(abs.(S_NMM_fcc[365,:,:] - I(14))),  
#       #     clims=(-10, 1), title=L"\mathrm{FCC}\; \mathrm{Onsite}\;  \log|S - \tilde{S}|", titlefont = font(12)), 
#            heatmap!(p8, log10.(abs.(H_NMM_fcc[366,:,:] - Hp_NMM_fcc[366,:,:])),  
#               colorbar=false, clims=(-10, 1), title=L"\mathrm{FCC}\; \mathrm{Offsite}\;  \log|H - \tilde{H}|", titlefont = font(12)),
#            heatmap!(p9, log10.(abs.(S_NMM_fcc[366,:,:] - Sp_NMM_fcc[366,:,:])), 
#               colorbar=false, clims=(-10, 1), title=L"\mathrm{FCC}\; \mathrm{Offsite}\;  \log|S - \tilde{S}|", titlefont = font(12)),    

#     heatmap!(p1, log10.(abs.(H_NMM_bcc[365,:,:])),  
#               colorbar=false, clims=(-10, 1), title=L"\mathrm{BCC}\; \mathrm{Onsite}\; \log|H|", titlefont = font(12)),
#     #  heatmap(log10.(abs.(S_NMM[365,:,:])),  
#     #           clims=(-10, 1), title=L"\mathrm{BCC}\; \mathrm{Onsite}\;  \log|S|", titlefont = font(12)), 
#      heatmap!(p2, log10.(abs.(H_NMM_bcc[366,:,:])),  
#              colorbar=false, clims=(-10, 1), title=L"\mathrm{BCC}\; \mathrm{Offsite}\;  \log|H|", titlefont = font(12)),
#      heatmap!(p3, log10.(abs.(S_NMM_bcc[366,:,:])), 
#               colorbar=false, clims=(-10, 1), title=L"\mathrm{BCC}\; \mathrm{Offsite}\;  \log|S|", titlefont = font(12)),

#               heatmap!(p10, log10.(abs.(H_NMM_bcc[365,:,:] - Hp_NMM_bcc[365,:,:])), aspect_ratio=:equal,  
#               colorbar=false, clims=(-10, 1), title=L"\mathrm{BCC}\; \mathrm{Onsite}\; \log|H - \tilde{H}|", titlefont = font(12)),
#           #  heatmap(log10.(abs.(S_NMM_bcc[365,:,:] - I(14))), aspect_ratio=:equal,  clims=(-10, 1), title=L"\mathrm{BCC}\; \mathrm{Onsite}\;  \log|S - \tilde{S}|", titlefont = font(12)), 
#            heatmap!(p11, log10.(abs.(H_NMM_bcc[366,:,:] - Hp_NMM_bcc[366,:,:])), aspect_ratio=:equal,  
#               colorbar=false, clims=(-10, 1), title=L"\mathrm{BCC}\; \mathrm{Offsite}\;  \log|H - \tilde{H}|", titlefont = font(12)),
#            heatmap!(p12, log10.(abs.(S_NMM_bcc[366,:,:] - Sp_NMM_bcc[366,:,:])), colorbar=false,
#               clims=(-10, 1), title=L"\mathrm{BCC}\; \mathrm{Offsite}\;  \log|S - \tilde{S}|", titlefont = font(12)),
      
#     aspect_ratio=:equal, size=(800, 1200), layout=(4, 3))
...
E_band = []
for i = 0:15
    filename = "fcc-to-bcc/out_$(@sprintf "%03d" i)_2_9.h5"
    ...
    push!(E_band, sum(fermi.(ϵp_kM, ϵf_p, 0.01) .* ϵ_kM_fcc .* w_k))
    push!(dos, d_p)
    push!(efermi, ϵf_p)
    bands = bandstructure(bcc_kpath, H, S, R)
    push!(allbands, bands)
end

##

fcc_atoms = load_json("test/FCC-999-supercell.json")
bcc_atoms = load_json("test/BCC-999-supercell.json")

atoms = [ load_json("fcc-to-bcc/geometry-$(@sprintf "%03d" i).json") for i=0:15 ]
...
function plot_fcc_bcc(i)
    plt = plot((egrid .- ϵf_fcc) .* hartree2ev, d_fcc_fine, color=3, label="FCC")
    plot!((egrid .- ϵf_bcc) .* hartree2ev, d_bcc_fine, color=4, label="BCC")
    plot!((egrid .- efermi[i+1]) .* hartree2ev, dos[i+1], color=:blue,
           label=raw"$c/a = " * @sprintf("%.2f",c_over_a(i+1)) * raw"$")
    return xlims!(-10,5)
end

plt = plot([plot_fcc_bcc(i) for i in 0:15]..., layout=(4, 4), size=(800, 800))
savefig("BCC_to_FCC_path.pdf")
plt
##
...
vline!([1.0], color=3, ls=:dash, label="FCC")
vline!([sqrt(2)], color=4, ls=:dash, label="BCC")
vline!([ca[5+1], ca[8+1]], color=:blue, ls=:dash, label="")
 
plt = plot(plot(ylabel!(plot_fcc_bcc(12), "DoS"), 
                  plot_fcc_bcc(8), plot_fcc_bcc(5),
                  plot_fcc_bcc(2), layout=(1, 4)),
    dos_diff, layout=(2, 1))

savefig("BCC_to_FCC.pdf")
plt
