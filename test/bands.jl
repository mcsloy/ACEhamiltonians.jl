...
function split_HS(Hb, Sb, R)
    ...
    H_NMM = zeros(size(R, 1), N_orb, N_orb)
    S_NMM = zeros(size(R, 1), N_orb, N_orb)
    ia = findfirst(all(R .== [0 0 0], dims=2))[1]
    blocks = Tuple{Int64,Int64}[]
    for ja = 1:blocksize(Hb,1)
        push!(blocks, (ia, ja))
...
function plot_bands(kpath::KPath, bands::Matrix; ϵ=nothing, fermi_level=nothing, 
                    plotobj=nothing, label=nothing, color=:black, ylabel="Energy (eV)")
    k0 = 0.0
    x_k = []
    tick_p = [k0]
    ...
    fermi_level !== nothing && plot!(plotobj, [fermi_level], lw=1, c=color, ls=:dash, seriestype=:hline, label=false)
