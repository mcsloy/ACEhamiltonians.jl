...
"""
    ParaDef(ν, deg, e_cutₒᵤₜ, e_cutᵢₙ[, λₙ, λₗ])
...
by providing a dictionary valued by a single scaler rather than a matrix:
```
...
mutable struct ParaDef{Ti,Tf}
    ...
    function ParaDef(ν::Ti, deg::Ti, e_cutₒᵤₜ::Tf, e_cutᵢₙ::Tf, bond_cut::Tf, λₙ::Tf, λₗ::Tf
        ...
        # Verify interactions are not doubly defined; i.e. if C-H is present H-C is not.
        if ~all(.==((i -> length.(keys.(i))).((args, sorted_args))...))
            throw(AssertionError("Off-site dictionaries must not contain symmetrically "*
    ...
    function ParaDef(ν::Ti, deg::Ti, e_cutₒᵤₜ::Tf, e_cutᵢₙ::Tf
        ...
        if length(non_sym_keys) != 0
            throw(AssertionError("Non-symmetric parameter matrices not permitted for on"*
            "-site interactions: $(join(non_sym_keys, ", "))"))
        end

        new{Ti, Tf}(ν, deg, e_cutₒᵤₜ, e_cutᵢₙ, nothing, nothing, nothing)
    end

    """Parameter expander constructor"""
    function ParaDef(basis_def::BasisDef, ν::P, deg::P, e_cutₒᵤₜ::P=12.0, e_cutᵢₙ::P=2.0,
        ...
        if site == "auto"
            ...
            if any([keytype(i)<:Tuple for i in c_args if isa(i, Dict)])
                site = "off"

            # If all arguments are scaler com_args then it is not possible to auto-determine site
            elseif all([isa(i, Real) for i in c_args])
                throw(AssertionError("Site cannot be auto-resolve if all mutual arguments are"*
...
function _parameter_expander(parameters::Union{Real, Dict{K, A}, Nothing}, basis_def::BasisDef,
    ...
    if isa(parameters, Real)
        ...
        elseif site == "off"
            ...
            throw(AssertionError("Site cannot be auto-resolve for global declarations."))
        end
    end
...
function _parameter_expander(
    ...
    for (i, (species, parameter)) in enumerate(parameters)
        ...
        if isa(parameter, Vector) parameter = _vector_to_matrix(parameter) end
        ...
        if isa(parameter, Real)  # Species resolved declarations
        ...
        elseif length(parameter) != ns₁ * ns₂
            ...
            site = isa(species, Tuple) ? "off" : "on"
            throw(AssertionError(
                "Malformed $site-site parameter matrix ($sg) given for $sp; expected"*
